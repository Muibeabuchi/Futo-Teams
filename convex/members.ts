import {authenticatedUserMutation, authenticatedUserQuery} from "./middleware";
...
export const remove = authenticatedUserMutation({
    ...
    // grab the member to be deleted
        ...
        if(!memberToBeDeleted) throw new ConvexError("Error while deleting member");

    //     grab the users member data
        const userMember = await ctx.db.query("members").withIndex("by_userId_by_workspaceId",q=>q.eq("userId",ctx.userId).eq("workspaceId",memberToBeDeleted.workspaceId)).unique()
        if(!userMember) throw new ConvexError("Unauthorized");

    //     grab the length of all the members in the workspace
        const allMembers = await ctx.db.query("members").withIndex("by_workspaceId",q=>q.eq("workspaceId",memberToBeDeleted.workspaceId)).collect()

    //     if we are trying to delete another user, we must be admins
        if(userMember._id !== memberToBeDeleted._id && userMember.role !== "admin") throw new ConvexError("Unauthorized")

    //     prevent user from deleting himself if he is the last member in the workspace
        if(allMembers.length <=1) throw new  ConvexError("There must be at least 1 member in the workspace")
...
export const updateRole = authenticatedUserMutation({
    ...
    async handler(ctx,args){
        ...
        if(!memberToBeEdited) throw new ConvexError("Error while updating the member");


        //     grab the users member data
        const userMember = await ctx.db.query("members").withIndex("by_userId_by_workspaceId",q=>q.eq("userId",ctx.userId).eq("workspaceId",memberToBeEdited.workspaceId)).unique()
        if(!userMember) throw new ConvexError("Unauthorized");

        //     grab the length of all the members in the workspace
        const allMembers = await ctx.db.query("members").withIndex("by_workspaceId",q=>q.eq("workspaceId",memberToBeEdited.workspaceId)).collect()
        ...
        // prevent user from deleting himself if he is the last member in the workspace
        if(allMembers.length <=1) throw new  ConvexError("Cannot downgrade the only member in the workspace")
